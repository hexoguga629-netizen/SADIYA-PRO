...
export default function registerSADIYACoder({ ipcMain, app }: { ipcMain: IpcMain; app: App }) {
  ...
  ipcMain.handle('start-live-coding', async (event, { prompt, filename, geminiKey }) => {
    try {
      ...
      const response = await ai.models.generateContentStream({
        ...
        contents: `You are an elite developer. Write the code for: "${prompt}". Output ONLY the raw code for the file ${filename}. No markdown.`
      })
