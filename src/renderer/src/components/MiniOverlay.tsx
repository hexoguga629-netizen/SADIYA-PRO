...
import { sadiyaService } from '@renderer/services/Sadiya-voice-ai'
...
const MiniOverlay = ({
  ...
  useEffect(() => {
    if (isSystemActive && sadiyaService.analyser) {
      analyzerRef.current = sadiyaService.analyser
      dataArrayRef.current = new Uint8Array(sadiyaService.analyser.frequencyBinCount)
      const checkAudio = () => {
    ...
    <div className="w-full h-full flex items-center justify-between px-3 bg-zinc-950/90 backdrop-blur-xl rounded-full border border-emerald-500/30 drag-region overflow-hidden">
      ...
      <div className="flex items-center gap-2 no-drag">
        <button
          ...
          className={`p-2.5 rounded-full transition-all ml-1 pointer-events-auto ${!isSystemActive ? 'text-zinc-400 bg-zinc-800/80 border border-zinc-700/50' : isMicMuted ? 'text-red-500 bg-red-500/10' : 'text-emerald-400 bg-emerald-500/10'}`}
          title={isSystemActive ? 'Toggle microphone' : 'Start SADIYA voice'}
        >
