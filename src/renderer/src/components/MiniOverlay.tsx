...
const MiniOverlay = ({
  ...
  return (
    <div className="w-full h-full flex items-center justify-between px-3 bg-zinc-950/90 backdrop-blur-xl rounded-full border 
      <div className="flex items-center gap-3 no-drag">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all duration-300 ${isSystemActi
        >
          <div
            className={`w-2.5 h-2.5 rounded-full transition-colors duration-300 ${isSystemActive ? (isTalking ? 'bg-emerald-4
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 no-drag">
        <button
          ...
          className={`p-2.5 rounded-full transition-all ml-1 ${!isSystemActive ? 'opacity-30' : isMicMuted ? 'text-red-500 bg
        >
        ...
        <button
          ...
          className={`p-3 rounded-full border transition-all duration-500 shadow-lg mx-1 ${isSystemActive ? 'bg-emerald-500/2
        >
        ...
        <button
          ...
          disabled={!isSystemActive}
          className={`p-2.5 rounded-full transition-all ${!isSystemActive ? 'opacity-30' : isVideoOn && visionMode === 'camer
          title="Toggle Camera"
        >
        ...
        <button
          ...
          disabled={!isSystemActive}
          className={`p-2.5 rounded-full transition-all ${!isSystemActive ? 'opacity-30' : isVideoOn && visionMode === 'scree
          title="Toggle Screen"
        >
    ...
    <div className="flex flex-col gap-2 relative w-full">
      <div className="relative w-full">
        ...
        <input
          ...
          className="w-full bg-[#09090b] border border-[#27272a] rounded-md text-xs py-2 pl-9 pr-3 text-white outline-none fo
        />
      </div>

      <div className="flex flex-col gap-1.5 h-120 overflow-y-auto scrollbar-small bg-[#09090b] border border-[#27272a] rounde
        ...
        {visibleApps.map((app, index) => {
          ...
          const AppRow = (
            <div
              ...
              className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-all duration-300 transfo
            >
...
export default function ParameterEditorDrawer({ nodeData, updateNodeInputs, closeEditor }: any) {
  ...
  return (
    <div className="absolute top-0 right-0 w-80 h-full bg-[#111113] border-l border-[#27272a] shadow-2xl flex flex-col z-50 a
      ...
      <div className="p-5 grow overflow-y-auto flex flex-col gap-6 custom-scrollbar scrollbar-small">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            className="bg-[#09090b] border border-[#27272a] rounded-md text-xs p-2.5 text-white outline-none focus:border-eme
            value={localComment}
        ...
        <div className="flex flex-col gap-4">
          ...
          {tool.parameters?.properties && Object.keys(tool.parameters.properties).length > 0 ? (
            Object.entries(tool.parameters.properties).map(([key, prop]: any) => (
              <div key={key} className="flex flex-col gap-2 w-full">
                ...
                {key === 'app_name' && (tool.name === 'open_app' || tool.name === 'close_app') ? (
                  ...
                  <select
                    className="bg-[#09090b] border border-[#27272a] rounded-md text-xs p-2.5 text-white outline-none focus:bo
                    value={localInputs[key] || ''}
            ...
            <p className="text-[10px] text-zinc-600 italic uppercase tracking-widest bg-black/30 p-2 rounded text-center bord
      ...
      <div className="p-4 border-t border-[#27272a] bg-[#18181b]">
        <button
          onClick={handleSave}
          className="w-full bg-emerald-600 hover:bg-emerald-500 text-black py-2.5 rounded-lg text-[11px] font-black tracking-
        >
