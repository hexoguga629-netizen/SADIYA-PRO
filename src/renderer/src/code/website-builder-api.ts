  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className={`flex items-center gap-3 px-4 py-2 bg-[#18181b] border rounded-lg text-sm text-zinc-300 font-medium transi
      >
      ...
      {isMainOpen && (
        <div className="absolute top-12 left-0 w-80 bg-[#111113] border border-[#27272a] rounded-xl shadow-2xl z-50 p-2 flex 
          ...
          {workflows.map((macro: any) => (
            <div key={macro.name} className="relative group">
              <button
                ...
                className="w-full text-left flex flex-col gap-1 p-3 rounded-lg hover:bg-zinc-800/60 group cursor-pointer bord
              >
              ...
              <button
                ...
                className="absolute top-3 right-3 p-1 rounded-md text-zinc-700 hover:text-white hover:bg-zinc-700 group curso
              >
                <RiMore2Fill size={16} />
              </button>

              {activeWorkflowActions === macro.name && (
                <div className="absolute top-8 right-2 w-32 bg-black border border-[#27272a] rounded-lg shadow-xl z-20 p-1 fl
                  {[
                    ...
                    <button
                      key={btn.label}
                      onClick={btn.action}
                      className={`flex items-center gap-2 p-2 rounded text-[10px] uppercase font-bold text-zinc-300 hover:bg-
                    >
                      {btn.icon} {btn.label}
                    </button>
                  ))}
